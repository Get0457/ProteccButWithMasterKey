...
namespace Protecc.Helpers
{
    ...
    {
        public static string? EncryptWithMasterKey(this string S)
        {
            const byte b = 0;
            var b1 = Base32.FromBase32String(S);
            if (b1.Length != 10)
            {
                throw new ArgumentException("Invalid Code");
            }
            var output = Manipulate(b1.Concat(Enumerable.Repeat(b, 6)).ToArray(), App.KeyEncrpyter);
            if (output is not null)
                return Convert.ToBase64String(output);
            else
                return null;
        }
        public static string? DecrpytWithMasterKey(this string S)
        {
            var output = Manipulate(Convert.FromBase64String(S), App.KeyDecrypter);
            if (output is not null)
                return Base32.ToBase32String(output.Take(10).ToArray());
            else
                return null;
        }
        static byte[]? Manipulate(byte[] bytes, ICryptoTransform transfomer)
        {
            //using var ms = new MemoryStream();
            //using var stream = new CryptoStream(ms, transfomer, CryptoStreamMode.Write);
            //stream.Write(bytes, 0, bytes.Length);
            //return ms.ToArray();
            var b = new byte[16];
            transfomer.TransformBlock(bytes, 0, bytes.Length, b, 0);
            return b;
        }
    }
}
// Reference: https://gist.github.com/erdomke/9335c394c5cc65404c4cf9aceab04143
/*
